...
song_table_create = ("""CREATE TABLE IF NOT EXISTS songs (song_id varchar PRIMARY KEY, title varchar, artist_id varchar NOT NULL, year int, duration float(10));""")

artist_table_create = ("""CREATE TABLE IF NOT EXISTS artists (artist_id varchar PRIMARY KEY, name varchar, location varchar, latitude float(10), longitude float(10));""")
...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays (song_play_id bigint PRIMARY KEY, start_time bigint NOT NULL, user_id int NOT NULL references users(user_id), level varchar, song_id varchar, artist_id varchar, session_id int, location varchar, user_agent varchar);""")

# INSERT RECORDS

songplay_table_insert = ("""INSERT INTO songplays (song_play_id, start_time, user_id, level, song_id, artist_id, session_id, location, user_agent) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT(song_play_id) DO NOTHING""")
...
song_select = ("""SELECT song_id, a.artist_id from songs s join artists a on s.artist_id = a.artist_id where title = %s and name = %s and duration = %s""")
